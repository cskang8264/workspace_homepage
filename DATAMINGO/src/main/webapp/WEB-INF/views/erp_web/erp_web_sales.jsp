...
<%@ include file="../include/nav.jsp" %> --%>
	...
	<div class="container text-center">  
			...
			<table class="table table-hover">
				<thead>       	   
					<tr>   
						...
						<th scope="col"><a href="/erp/web/sales" class="btn btn-primary btn-lg btn-block" role="button" aria-pressed="true">매출</a></th>
					</tr>
				</thead>   
			</table>   
		<BR>      
		<p class="lead">판매 수동 입력</p>
		...
		<tbody>
			<c:forEach var="s" items="${sales}" varStatus="status">	
				<tr>  
					<td>${s.product_no}</td>
					<td>${s.sales_cnt}</td>
					<td>${s.sales_timestamp}</td>
					<td><button type="button" class="btn btn-outline-dark" onclick="DELETE(${s.sales_pk},${s.sales_cnt},'${s.product_no}')">삭제</button></td>
				</tr>
			</c:forEach>
		</tbody>	
		</table>      
